...
export default function LabPage() {
  ...
  const collectPayment = () => {
    ...
    toast.success('ادائیگی ہو گئی - ٹیسٹ شروع کریں');
  };
  ...
  const togglePaymentLocation = (val: boolean) => {
    ...
    saveData(updated);
    setData(updated);
    toast.success(`ادائیگی: ${val ? 'لیب' : 'ریسپشن'}`);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        <div className="grid grid-cols-3 gap-4 mb-6">
          <Card>
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div><p className="text-2xl font-bold">{orderedTests.length}</p><p className="text-xs text-gray-500">آرڈرڈ</p></div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div><p className="text-2xl font-bold">{paidTests.length}</p><p className="text-xs text-gray-500">ادا شدہ</p></div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div><p className="text-2xl font-bold">{completedTests.length}</p><p className="text-xs text-gray-500">مکمل</p></div>
            </CardContent>
          </Card>
        </div>
        ...
        <Card className="mb-6">
          <CardContent className="p-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm font-medium">لیب</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-lg">لیب ٹیسٹس</CardTitle>
          </CardHeader>
          <CardContent>
            {allLabOrders.length === 0 ? (
              <p className="text-gray-500 text-center py-8">کوئی ٹیسٹ نہیں</p>
            ) : (
                    ...
                    <div key={order.id} className={`border rounded-lg p-4 flex items-center justify-between ${blocked ? 'bg-amber-50 border-amber-200' : 'hover:bg-gray-50'}`}>
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          {blocked && <p className="text-xs text-amber-600 mt-1">ریسپشن سے ادائیگی کا انتظار</p>}
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        <Badge variant={order.status === 'completed' ? 'default' : 'outline'}
                          className={order.status === 'completed' ? 'bg-emerald-600' : ''}>
                          {order.status === 'ordered' ? 'آرڈرڈ' : order.status === 'paid' ? 'ادا شدہ' : order.status === 'processing' ? 'پروسسنگ' : 'مکمل'}
                        </Badge>
                        {!blocked && (
              ...
              <>
                ...
                <div className="space-y-4">
                  <div className="bg-gray-50 rounded-lg p-4 text-sm space-y-1">
                    ...
                    <p>پٹینٹ نمبر: {selectedOrder.visit.patientNumber}</p>
                    <p>وزیٹ: {selectedOrder.visit.visitNumber}</p>
                    <p>قیمت: Rs. {selectedOrder.order.price}</p>
                    <p>حالت: {selectedOrder.order.status}</p>
                  </div>
                  {selectedOrder.order.result && (
                    <div className="bg-emerald-50 rounded-lg p-4">
                      <p className="text-sm font-bold mb-1">نتائج:</p>
                ...
                <DialogFooter>
                  ...
                  {selectedOrder.order.status === 'paid' && (
                    <Button onClick={() => { setResultText(selectedOrder.order.result); setShowResult(true); }} className="bg-orange-600 hover:bg-orange-700">
                      <Beaker className="w-4 h-4 mr-1" /> نتائج درج کریں
                    </Button>
                  )}
                </DialogFooter>
              </>
        ...
        <Dialog open={showPayment} onOpenChange={setShowPayment}>
          <DialogContent>
            ...
            <p className="text-sm">Rs. {selectedOrder?.order.price} - ادائیگی کی تصدیق کریں</p>
            <DialogFooter>
        ...
        <Dialog open={showResult} onOpenChange={setShowResult}>
          <DialogContent>
            <DialogHeader><DialogTitle>ٹیسٹ کے نتائج</DialogTitle></DialogHeader>
            <Textarea value={resultText} onChange={e => setResultText(e.target.value)} rows={6} placeholder="نتائج درج کریں..." />
            <DialogFooter>
