...
const roleMenus: Record<string, RoleMenu> = {
  ...
  pharmacy: [
    { label: 'Pharmacy Dashboard', path: '/pharmacy', icon: 'M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zm10 0a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zm10 0a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z' },
    { label: 'Point of Sale', path: '/pharmacy/pos', icon: 'M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 100 4 2 2 0 000-4z' },
...
function getPageTitle(pathname: string, menuItems: RoleMenu, searchStr: string): string {
  ...
  if (pathname.startsWith('/pharmacy/')) {
    const subMap: Record<string, string> = {
      ...
      '/pharmacy/reports': 'Sales Reports',
      '/pharmacy/expenses': 'Expenses',
      '/pharmacy/statement': 'My Statement',
    };
    return subMap[pathname] || 'Pharmacy';
  }
...
export default function AppLayout({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const savedDark = localStorage.getItem('baga_dark_mode');
    if (savedDark === 'true') {
      setDarkMode(true);
  ...
  const toggleDarkMode = () => {
    ...
    setDarkMode(newMode);
    localStorage.setItem('baga_dark_mode', String(newMode));
    if (newMode) {
      document.documentElement.classList.add('dark');
    } else {
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      ...
      if (!target.closest('[data-notifications-panel]') && !target.closest('[data-notifications-btn]')) {
  ...
  if (lt === 'pharmacy') {
    // Pharmacy license: simplified POS with all pharmacy-relevant pages
    menuItems = [
      { label: 'Pharmacy Dashboard', path: '/pharmacy', icon: 'M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zm10 0a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zm10 0a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z' },
      { label: 'Point of Sale', path: '/pharmacy/pos', icon: 'M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 100 4 2 2 0 000-4z' },
      { label: 'Medicine Inventory', path: '/pharmacy/inventory', icon: 'M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10' },
    ...
    menuItems = [
      { label: 'Dashboard', path: '/lab', icon: 'M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zm10 0a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zm10 0a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z' },
      {
        ...
        children: [
          ...
          { label: 'Test Catalog', path: '/lab/settings', icon: 'M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.066 2.573c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.573 1.066c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.066-2.573c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z' },
          { label: 'Expenses', path: '/lab/expenses', icon: 'M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z' },
          { label: 'Lab Doctors', path: '/lab/doctors', icon: 'M5.121 17.804A13.937 13.937 0 0112 16c2.5 0 4.847.655 6.879 1.804M15 10a3 3 0 11-6 0 3 3 0 016 0zm6 2a9 9 0 11-18 0 9 9 0 0118 0z' },
          { label: 'My Statement', path: '/lab/statement', icon: 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z' },
        ],
      },
      { label: 'User Management', path: '/users', icon: 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z' },
  ...
  return (
    <div className={`flex min-h-screen ${darkMode ? 'dark bg-slate-900' : 'bg-slate-100'}`}>
      {sidebarOpen && <div className="fixed inset-0 bg-black/50 z-40 lg:hidden" onClick={() => setSidebarOpen(false)} />}

      {/* Sidebar */}
      <aside className={`sidebar ${sidebarOpen ? 'open' : ''} ${darkMode ? 'dark-sidebar' : ''}`}>
        <div className="p-3 border-b border-slate-700 flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-slate-400 text-xs">{roleLabels[session.role]}</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-3 border-t border-slate-700 flex-shrink-0">
          <div className="flex items-center gap-2 mb-2">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-slate-400 text-xs">{session.department}</p>
            </div>
          </div>
          {session?.mode === 'demo' && licenseInfo?.demo && (
            <div className="bg-amber-600/20 border border-amber-500/30 rounded-lg px-3 py-2 mb-2">
              <p className="text-amber-400 text-xs font-semibold text-center">Demo Mode</p>
              <p className="text-amber-300 text-[10px] text-center">{licenseInfo.demo.remaining} day(s) remaining</p>
            </div>
          )}
      ...
      {/* Main */}
      <div className={`main-content flex-1 ${darkMode ? 'dark-main' : ''}`}>
        <header className={`border-b px-6 py-3 flex items-center justify-between sticky top-0 z-30 ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
          <div className="flex items-center gap-4">
            ...
            <h1 className={`text-lg font-semibold ${darkMode ? 'text-white' : 'text-slate-800'}`}>{currentLabel}</h1>
          </div>
          <div className="flex items-center gap-3">
            {/* Notification Bell */}
            <div className="relative">
              <button data-notifications-btn onClick={() => setShowNotifications(!showNotifications)} className={`p-2 rounded-lg transition-colors ${darkMode ? 'hover:bg-slate-700' : 'hover:bg-slate-100'}`}>
              ...
              {showNotifications && (
                <div data-notifications-panel className={`absolute right-0 top-12 w-80 rounded-xl shadow-2xl border z-50 ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
                  <div className={`px-4 py-3 border-b font-semibold text-sm ${darkMode ? 'border-slate-700 text-white' : 'border-slate-100 text-slate-800'}`}>
                    Notifications
                  </div>
                  <div className="p-3">
                    {updateAvailable ? (
            ...
            {/* Dark Mode Toggle */}
            <button onClick={toggleDarkMode} className={`p-2 rounded-lg transition-colors ${darkMode ? 'hover:bg-slate-700' : 'hover:bg-slate-100'}`} title="Toggle Dark Mode">
              <svg className={`w-5 h-5 ${darkMode ? 'text-yellow-400' : 'text-slate-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
