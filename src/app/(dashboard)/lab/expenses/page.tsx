...
import { initLabData, getLabExpenses, addExpense, updateExpense, deleteExpense, genId, todayStr } from '@/lib/lab-store';
import { getHospitalSettings } from '@/lib/store';
import type { LabExpense } from '@/lib/lab-store';

const CATEGORIES = ['Reagents', 'Consumables', 'Equipment', 'Maintenance', 'Waste Management', 'Utilities', 'Salaries', 'Miscellaneous'];
const DATE_FILTERS = ['Today', 'This Week', 'This Month', 'This Year', 'All'] as const;

export default function LabExpensesPage() {
  ...
  const [form, setForm] = useState({ description: '', category: '', amount: '', date: '', notes: '' });
  ...
  const filterByDate = (items: LabExpense[]) => {
  ...
  const filtered = dateFiltered.filter(e => {
    ...
    const matchSearch = search === '' || e.description.toLowerCase().includes(search.toLowerCase()) || e.category.toLowerCase().includes(search.toLowerCase()) || e.notes.toLowerCase().includes(search.toLowerCase());
  ...
  const openAddModal = () => {
    setEditingId(null);
    setForm({ description: '', category: CATEGORIES[0], amount: '', date: todayStr(), notes: '' });
    setShowModal(true);
  };

  const openEditModal = (item: LabExpense) => {
    setEditingId(item.id);
    setForm({ description: item.description, category: item.category, amount: String(item.amount), date: item.date, notes: item.notes });
    setShowModal(true);
  };
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">Track and manage lab expenses</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="stat-card card-hover border border-teal-200 bg-teal-50">
          <p className="text-xs text-teal-600 uppercase tracking-wide font-semibold">Total Expenses</p>
          <p className="text-2xl font-bold text-teal-700 mt-1">{currency} {totalExpenses.toLocaleString()}</p>
        </div>
        ...
        <div className="stat-card card-hover border border-amber-200 bg-amber-50">
          <p className="text-xs text-amber-600 uppercase tracking-wide font-semibold">Categories</p>
          <p className="text-2xl font-bold text-amber-700 mt-1">{catCount}</p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="data-table">
            <thead>
              <tr><th>#</th><th>Date</th><th>Description</th><th>Category</th><th>Amount</th><th>Notes</th><th>Actions</th></tr>
            </thead>
            <tbody>
              {filtered.map((item, idx) => (
                <tr key={item.id}>
                  ...
                  <td className="font-bold text-teal-700">{currency} {item.amount.toLocaleString()}</td>
                  <td className="text-sm text-slate-500 max-w-48 truncate">{item.notes || '—'}</td>
                  <td>
          ...
          <div className="modal-content" style={{ maxWidth: '500px' }} onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input className="form-input" value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} placeholder="e.g. Reagent kit purchase" />
              </div>
